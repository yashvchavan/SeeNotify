...
const IntegrationScreen = () => {
  ...
  const toggleAppConnection = (id:any) => {
    setConnectedApps(connectedApps.map((app) => (app.id === id ? { ...app, isConnected: !app.isConnected } : app)))
  }

  type PermissionKey = "readEmails" | "readAttachments" | "sendReplies" | "readMessages" | "readGroups" | "readEvents" | "createEvents" | "sendReminders";
  
  const togglePermission = (appId: string, permission: PermissionKey) => {
      setConnectedApps(
                ...
                <View style={styles.permissionsContainer}>
                  ...
                  {Object.entries(app.permissions).map(([key, value]) => (
                    <View key={key} style={styles.permissionItem}>
                      ...
                      <Switch
                        value={value}
                        onValueChange={() => togglePermission(app.id, key as PermissionKey)}
                        trackColor={{
                          false: isDark ? "#2e2e3e" : "#e2e8f0",
                          true: isDark ? "#4f46e5" : "#6366f1",
                        }}
